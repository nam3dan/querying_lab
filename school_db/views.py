...
def problem_one(request):
    students_rank = Student.objects.filter(gpa__gt=3.0).order_by('-gpa')
...
def problem_two(request):
    instructors = Instructor.objects.filter(hire_date__iso_year__lt=2010).order_by('hire_date')

    for instuctor in instructors:
      print(
          f'''Full Name: {instuctor.first_name} {instuctor.last_name} 
Hire Date: {instuctor.hire_date}''')
...
def problem_five(request):
    me = Student.objects.create(first_name="Dan", last_name="Gehring", year=17,gpa=4.0)
    print(f"""
    Id: {me.id}
...
def problem_six(request):
    ...
    student_id = 11
    update = Student.objects.filter(id=11).update(gpa=5.0)
...
def bonus_problem(request):
    ...
    for i in range(1,instructor_count+1):
        if Course.objects.filter(instructor_id=i).count() == 1:
            lazy_teacher = Instructor.objects.get(id=i)
            print(f" Instructor Name: {lazy_teacher.first_name} {lazy_teacher.last_name}")
        else:
            pass
